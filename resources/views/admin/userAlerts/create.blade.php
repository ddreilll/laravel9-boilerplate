    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.user-alerts.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label class="required" for="alert_content">{{ trans('cruds.userAlert.fields.alert_content') }}</label>
                    <textarea id="summernote" class="form-control {{ $errors->has('alert_content') ? 'is-invalid' : '' }}" type="text"
                        name="alert_content" id="alert_content" value="{{ old('alert_content', '') }}">
                </textarea>
                    @if ($errors->has('alert_content'))
                        <span class="text-danger">{{ $errors->first('alert_content') }}</span>
                    @endif
                    <span class="help-block">{{ trans('cruds.userAlert.fields.alert_content_helper') }}</span>
                </div>
                <div class="form-group">
